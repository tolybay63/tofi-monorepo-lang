...
public class UtPeriod {
    ...
    public XDate calcDbeg(XDate date, long periodType, int countPeriod) {
        ...
        } else if (periodType == FD_PeriodType_consts.quarteraccum) {
            res = calcDbegAccumulated(countPeriod, date);
    ...
    private XDate calcDbegAccumulated(int countPeriod, XDate dend) {
    ...
    private XDate calcDendQuarter(int lagCurrentDate, XDate date) {
        ...
        if (lagCurrentDate > 0) {
            ...
            res = curQuarterEnd.addMonths(Math.abs(lagCurrentDate) * 3).endOfMonth();
        }
